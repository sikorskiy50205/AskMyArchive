...
> AI-facing security checklist, который я (и модель, когда работает с кодом) читаю перед касанием чувствительной поверхности — в [SECURITY.md](SECURITY.md).
...
- Whitelist расширений загружаемых файлов; length-caps на весь пользовательский ввод (файл 50 МБ, OCR-текст 500 000 символов, чат-вопрос 4 000 символов); per-user квота хранилища 100 МБ.
...
**Content и LLM-специфичное**
- **Prompt injection в RAG-контексте.** `RagService` собирает system-prompt из имени файла и текста чанков, которые загружены пользователем — то есть из user-controlled ввода. Загруженный документ с содержимым «ignore previous instructions…» приведёт к self-jailbreak ассистента *на аккаунте того же пользователя*. Per-user изоляция ограничивает радиус — на чужие аккаунты повлиять нельзя. Продовое решение — обёртка вокруг user-content с явными делимитерами, escape служебных последовательностей в имени файла и cap длины включаемого текста; сейчас — осознанный трейд-офф между полнотой RAG и защитой от self-jailbreak на своём же аккаунте.
- **MIME-валидация файлов только по расширению**, не по magic bytes. Sniffing-XSS закрыт `X-Content-Type-Options: nosniff`; реальный риск ограничен тем, что PdfPig / ClosedXML на не-PDF/не-DOCX-содержимом бросит исключение и документ окажется в статусе `Failed`. Не уязвимость, но edge-case UX: файл `.pdf` с не-PDF байтами молча помечается как failed. Настоящий magic-bytes-check — TODO.
