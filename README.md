...
- API: http://localhost:8080 (Scalar UI: `/scalar/v1`)
...
Это портфолио-проект: цель — показать архитектуру RAG-сервиса и полный fullstack-цикл, а не поставить готовый к массовому трафику SaaS. Поэтому «продовое» усиление безопасности осознанно оставлено за периметром демо — то, что в бою должно быть выключено дефолтно и включаться отдельным prod-профилем, здесь работает в удобных настройках, чтобы `docker compose up` заводился с одной команды.

**Уже сделано «по-взрослому»** — переносится в прод как есть:

- JWT-access + refresh-ротация: single-use refresh, SHA-256-хэш в БД, ротация при каждом обновлении, инвалидация всех живых refresh'ов при смене пароля.
- PBKDF2-хеширование паролей (`PasswordHasher<AppUser>`), rate-limit на credential-эндпоинты (5/мин/IP), per-user изоляция во всех запросах к документам и чатам, whitelist расширений + квота на файлы, отказ стартовать вне Development с dev-JWT-ключом.
- Валидация Google ID-токена по audience, `HttpOnly` refresh-cookie с `Path=/api/auth`.

**Осознанно упрощено** — для «одна команда → работает локально»:

- Scalar/OpenAPI открыт безусловно (в проде — гейт `IsDevelopment()`).
- `docker-compose.yml` использует `ASPNETCORE_ENVIRONMENT: Development`, дефолтный пароль Postgres и публикует порт 5432 наружу.
- Refresh-cookie: `SecureCookie: false`, `SameSite: Lax` (в проде — `true` / `None` за HTTPS).
- Нет `UseForwardedHeaders` / `UseHsts` / `UseHttpsRedirection`; `AllowedHosts: "*"`.
- `/api/ask` без per-user rate-limit'а (в проде обязателен: LLM платный).
- Минимальный пароль — 8 символов, без HIBP-чека; регистрация не требует подтверждения email до первого логина.

Чек-лист боевого деплоя сводится к отдельному `docker-compose.prod.yml` + правкам в `Program.cs` строк на 20-30 — задача обозримая, но осознанно оставлена за скобками портфолио.
